...
def AddSalariedView(request, i):
    ...
    form = SalariedModelForm(
        initial={"type_employment": "SALARIED", "customer": customer}
    )
...
def AddSelfSalariedView(request, i):
    ...
    form = SelfSalariedModelForm(
        initial={"type_employment": "SELF-SALARIED", "customer": customer}
    )
    if request.method == "POST":
...
def page1(request, i):
    ...
    if request.method == "GET":
        ...
        if view == "salaried":
            return redirect(
                "/dapp/professional/addsal/%i" % customer.id
            )  # If you don't have a context, simply don't include it in this line
        elif view == "self employed":
            return redirect("/dapp/professional/addself/%i" % customer.id)

        else:
            render(
                request, "DashboardApp/Profe_detail.html"
            )  # If there's no match, load the default one.
